...
resource "aws_lambda_permission" "allow-ctbucket-trigger" {
  ...
  function_name = aws_cloudformation_stack.datadog-forwarder[var.aws_region].outputs.DatadogForwarderArn
...
resource "aws_s3_bucket_notification" "ctbucket-notification-dd-log" {
  ...
  lambda_function {
    lambda_function_arn = aws_cloudformation_stack.datadog-forwarder[var.aws_region].outputs.DatadogForwarderArn
